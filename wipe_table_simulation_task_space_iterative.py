...
class TaskSpaceSimulation:
    ...
    def __init__(self, model_path: str = "surface_force_control.xml"):
        ...
        print("✓ 空间任务空间控制仿真初始化完成（迭代优化版）")
        print(f"  迭代方法：{self.iterative_method}")
        print(f"  最大迭代次数：{self.max_iter}，收敛容差：{self.tol}")
        print(f"  步长：{self.alpha}，正则化参数：{self.lambda_reg}")
    ...
    def setup_control_parameters(self):
        ...
        # 迭代优化参数（方法4：投影梯度法）
        self.use_iterative = True  # 是否使用迭代优化（False则使用直接计算）
        self.iterative_method = 'projected_gradient'  # 迭代方法
        self.max_iter = 10  # 最大迭代次数（减少以提高实时性）
        self.tol = 1e-3  # 收敛容差（放宽以提高实时性）
        self.alpha = 0.5  # 步长（增大以加快收敛）
        self.lambda_reg = 0.001  # 正则化参数（减小以避免过度平滑）
    ...
    def iterative_torque_computation(self, F_s, J_s, tau_min, tau_max):
        """
        使用投影梯度法（方法4）迭代求解力矩
        
        优化问题：
            min_τ  (1/2) ||F_s - J_s^T(θ) τ||^2 + (λ/2) ||τ||^2
            s.t.   τ_min ≤ τ ≤ τ_max
        
        目标函数展开：
            J(τ) = (1/2) (F_s - J_s^T τ)^T (F_s - J_s^T τ) + (λ/2) τ^T τ
                 = (1/2) F_s^T F_s - F_s^T J_s^T τ + (1/2) τ^T J_s J_s^T τ + (λ/2) τ^T τ
                 = (1/2) F_s^T F_s - F_s^T J_s^T τ + (1/2) τ^T (J_s J_s^T + λ I) τ
        
        注意：J_s 是 6×n，J_s^T 是 n×6，所以：
        - J_s^T τ 是 6×1（正确）
        - J_s J_s^T 是 6×6（不能直接与τ相乘）
        
        正确的展开应该是：
            J(τ) = (1/2) F_s^T F_s - F_s^T J_s^T τ + (1/2) τ^T (J_s^T J_s) τ + (λ/2) τ^T τ
                 = (1/2) F_s^T F_s - F_s^T J_s^T τ + (1/2) τ^T (J_s^T J_s + λ I) τ
        
        梯度：
            ∇_τ J = -J_s F_s + (J_s^T J_s + λ I) τ
        
        参数:
            F_s: 任务空间控制力 (6,)
            J_s: 空间雅可比矩阵 (6, n_joints)
            tau_min: 最小力矩限制 (n_joints,)
            tau_max: 最大力矩限制 (n_joints,)
        
        返回:
            tau: 满足约束的最优力矩 (n_joints,)
            iter_count: 迭代次数
        """
        n_joints = J_s.shape[1]
        ...
        # 预计算矩阵（避免重复计算）
        # Hessian矩阵：H = J_s^T J_s + λI (n_joints × n_joints)
        H = J_s.T @ J_s + self.lambda_reg * np.eye(n_joints)
        # 常数项：b = J_s^T F_s (n_joints × 1)
        b = J_s.T @ F_s
        
        # 投影梯度法迭代
        for k in range(self.max_iter):
            # 计算梯度：g = -J_s^T F_s + (J_s^T J_s + λI) τ
            #            = -b + H @ tau
            g = -b + H @ tau
            
            # 梯度下降：τ_new = τ - α * g
            tau_new = tau - self.alpha * g
            
            # 投影到可行域：clip到力矩限制
            tau_new = np.clip(tau_new, tau_min, tau_max)
            
            # 检查收敛：梯度范数或变化量
            grad_norm = np.linalg.norm(g)
            tau_change = np.linalg.norm(tau_new - tau)
            
            tau = tau_new
            
            # 收敛判断
            if grad_norm < self.tol or tau_change < self.tol:
                return tau, k + 1
        
        # 达到最大迭代次数
        return tau, self.max_iter
    ...
    def control_step(self, t, dt=0.002):
        ...
        # 先直接计算
        tau_direct = J_s.T @ F_cmd
        
        if self.use_iterative:
            # 检查是否需要迭代优化（如果直接计算的结果已经满足约束，可能不需要迭代）
            if np.any(tau_direct < tau_min) or np.any(tau_direct > tau_max):
                # 力矩超出限制，使用迭代优化
                tau, iter_count = self.iterative_torque_computation(
                    F_cmd, J_s, tau_min, tau_max
                )
                
                # Debug输出
                if self.debug and (t - self.last_debug_time >= 1.0):
                    F_error = F_cmd - J_s.T @ tau
                    print(f"  Iter={iter_count} | ||F_error||={np.linalg.norm(F_error):.4f} | "
                          f"||tau-τ_direct||={np.linalg.norm(tau - tau_direct):.4f}")
            else:
                # 力矩在限制内，直接使用
                tau = tau_direct
                if self.debug and (t - self.last_debug_time >= 1.0):
                    print(f"  Direct computation (within limits)")
        else:
            # 直接计算（原始方法）
            tau = np.clip(tau_direct, tau_min, tau_max)
