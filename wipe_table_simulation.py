...
class WipeTableSimulation:
    ...
    def __init__(self, model_path: str = "surface_force_control.xml"):
        ...
        # 初始化 ikpy 链（如果可用，需要在设置初始配置之前）
        self.ikpy_chain = None
        self.ikpy_active_indices = None  # 活动关节在链中的索引
        if IKPY_AVAILABLE:
            try:
                # 尝试加载 URDF 文件
                urdf_paths = ["panda_arm.urdf", "panda.urdf", "franka_panda.urdf"]
                ...
                for urdf_path in urdf_paths:
                    try:
                        ...
                        for i, link in enumerate(temp_chain.links):
                            ...
                            if any(joint_name in link_name for joint_name in joint_names):
                        ...
                        if len(active_indices) == 7:
                            ...
                            self.ikpy_active_indices = active_indices
                ...
                if self.ikpy_chain is None:
                    print("⚠ 未找到合适的 URDF 配置，将使用备用方法计算姿态")
            except Exception as e:
                print(f"⚠ ikpy 初始化失败: {e}，将使用备用方法")
    ...
    def compute_initial_configuration_ik(self):
        ...
        try:
            # ========== 定义目标位姿 ==========
            # 目标位置：在表面上方
            target_position = np.array([0.5, 0.0, 0.3])  # [x, y, z]
            
            # 目标姿态：Z轴朝下
            # 构建旋转矩阵，使得末端Z轴指向 [0, 0, -1]（向下）
            # 方法：绕X轴旋转180度，使得Z轴从 [0, 0, 1] 变为 [0, 0, -1]
            # 旋转矩阵：R_x(180°) = [[1, 0, 0], [0, -1, 0], [0, 0, -1]]
            target_orientation = np.array([
                [1.0,  0.0,  0.0],  # X轴方向（保持不变）
                [0.0, -1.0,  0.0],  # Y轴方向（反向）
                [0.0,  0.0, -1.0]   # Z轴方向（向下）
            ])
            
            # ========== 准备初始猜测 ==========
            # 使用默认配置，但确保所有值都在合理范围内
            # 创建一个全零的初始猜测数组（对应所有链接）
            initial_guess = [0.0] * len(self.ikpy_chain.links)
            
            # 将默认配置映射到活动关节
            for i, active_idx in enumerate(self.ikpy_active_indices):
                if i < len(q_default) and active_idx < len(initial_guess):
                    # 确保值在合理范围内（避免超出关节限制）
                    angle = q_default[i]
                    # 限制在 [-π, π] 范围内
                    while angle > np.pi:
                    ...
                    while angle < -np.pi:
                        angle += 2 * np.pi
                    initial_guess[active_idx] = angle
            
            # ========== 使用 ikpy 求解逆运动学 ==========
            # 尝试不提供初始猜测，让 ikpy 自己选择
            try:
            ...
            q_init = np.zeros(7)
            for i, active_idx in enumerate(self.ikpy_active_indices):
                if i < len(q_init) and active_idx < len(ik_solution):
                    q_init[i] = ik_solution[active_idx]
            ...
            valid = True
            for i in range(7):
                if q_init[i] < joint_limits[i, 0] or q_init[i] > joint_limits[i, 1]:
                    valid = False
                    break
            
            if valid:
                print(f"✓ 通过IK求解得到初始配置: {q_init}")
                ...
                T_actual = self.ikpy_chain.forward_kinematics(joint_angles)
                pos_actual = T_actual[:3, 3]
                R_actual = T_actual[:3, :3]
                
                pos_error = np.linalg.norm(pos_actual - target_position)
                z_axis_actual = R_actual[:, 2]  # 实际Z轴方向
                z_axis_desired = target_orientation[:, 2]  # 期望Z轴方向
                orientation_error = np.linalg.norm(z_axis_actual - z_axis_desired)
                
                print(f"  位置误差: {pos_error:.4f} m")
                print(f"  姿态误差: {orientation_error:.4f}")
                print(f"  实际Z轴方向: {z_axis_actual}")
                print(f"  期望Z轴方向: {z_axis_desired}")
                
                if pos_error < 0.05 and orientation_error < 0.1:
                    print("  ✓ IK解验证通过")
                    return q_init
                else:
                    print(f"  ⚠ IK解验证失败，使用默认配置")
                    return q_default
            else:
                print("⚠ IK解超出关节限制，使用默认配置")
                return q_default
                
        except Exception as e:
            print(f"⚠ IK求解失败: {e}，使用默认初始配置")
            import traceback
            traceback.print_exc()
            return q_default
    ...
    def setup_control_parameters(self):
        ...
        self.K_i = np.diag([20.0, 20.0, 0.0])      # 积分增益
        self.K_d = np.diag([80.0, 80.0, 500.0])      # 微分增益
    ...
    def compute_end_effector_position_from_fk(self, q):
        ...
        返回:
            pos: 末端位置 (3,) [x, y, z]
        """
        if self.ikpy_chain is not None and self.ikpy_active_indices is not None:
            try:
                # 准备关节角度数组（长度等于链的链接数）
                joint_angles = [0.0] * len(self.ikpy_chain.links)
                ...
                # 提取位置（变换矩阵的最后一列的前3个元素）
                pos = T[:3, 3]
                return pos
            except Exception as e:
                print(f"⚠ ikpy 位置计算失败: {e}，使用 MuJoCo 数据")
        
        # 备用方法：使用 MuJoCo 数据
        try:
            return np.array(self.data.body("panda_hand").xpos)
        except:
            return np.array([0.5, 0.0, 0.3])  # 默认位置
    
    def compute_desired_orientation_from_fk(self, q, pos_ref):
        """
        通过正运动学计算期望的末端姿态四元数
        
        使用 ikpy 根据关节角度计算末端位姿
        ...
        if self.ikpy_chain is not None and self.ikpy_active_indices is not None:
            try:
                ...
                print(f"⚠ ikpy 正运动学计算失败: {e}，使用备用方法")
    ...
    def quaternion_error(self, q_ref, q_curr):
        ...
        返回:
            e_rot: 旋转误差向量 (3,)
        """
        # 四元数误差：q_err = q_ref ⊗ q_curr^{-1}
        # q_curr^{-1} = [q_curr[0], -q_curr[1], -q_curr[2], -q_curr[3]]
        q_curr_inv = np.array([q_curr[0], -q_curr[1], -q_curr[2], -q_curr[3]])
        
        # 四元数乘法
        q_err = np.array([
            q_ref[0]*q_curr_inv[0] - q_ref[1]*q_curr_inv[1] - q_ref[2]*q_curr_inv[2] - q_ref[3]*q_curr_inv[3],
            q_ref[0]*q_curr_inv[1] + q_ref[1]*q_curr_inv[0] + q_ref[2]*q_curr_inv[3] - q_ref[3]*q_curr_inv[2],
            q_ref[0]*q_curr_inv[2] - q_ref[1]*q_curr_inv[3] + q_ref[2]*q_curr_inv[0] + q_ref[3]*q_curr_inv[1],
            q_ref[0]*q_curr_inv[3] + q_ref[1]*q_curr_inv[2] - q_ref[2]*q_curr_inv[1] + q_ref[3]*q_curr_inv[0]
        ])
        
        # 转换为旋转向量（轴角表示）
        # 如果 q_err = [cos(θ/2), sin(θ/2)*n]，则旋转向量 = θ*n
        w = q_err[0]
        if abs(w) >= 1.0:
            e_rot = np.zeros(3)
        else:
    ...
    def control_step(self, t, dt=0.002):
        ...
        if not is_contact:
            ...
            trace = np.trace(R_des)
            if trace > 0:
                s = np.sqrt(trace + 1.0) * 2
                w = 0.25 * s
                x = (R_des[2, 1] - R_des[1, 2]) / s
                y = (R_des[0, 2] - R_des[2, 0]) / s
                z = (R_des[1, 0] - R_des[0, 1]) / s
            else:
                s = np.sqrt(1.0 + R_des[2, 2] - R_des[0, 0] - R_des[1, 1]) * 2
                w = (R_des[1, 0] - R_des[0, 1]) / s
                x = (R_des[0, 2] + R_des[2, 0]) / s
                y = (R_des[1, 2] + R_des[2, 1]) / s
                z = 0.25 * s
            
            quat_ref = np.array([w, x, y, z])
            ...
            # 使用正运动学重新计算姿态（基于当前关节角度和期望位置）
            if self.ikpy_chain is not None:
                quat_ref = self.compute_desired_orientation_from_fk(q, pos_ref)
            
            if self.quat_ref is None:
                self.quat_ref = quat_ref
